...
class TaxonomyExtractor:
    ...
    def extract_content(self,
        ...
        try:
            ...
            # For selective extraction, use selected_categories directly
            target_category_ids = set(selected_categories or [])
            safe_log_start(
            ...
            context_map = {}  # { parent_id: { 'info': parent_obj, 'children': [target_cat_1, target_cat_2] } }
                ...
                if current_category and hasattr(current_category, 'parent_path') and current_category.parent_path:
                    ...
                    for node in self._traverse_taxonomy(taxonomy):
                        if (hasattr(node, 'name') and node.name == immediate_parent_name and
                            ...
                            # FIX: Ensure parent_id is always a string for consistent lookup
                            ...
                            if parent_id in context_map:
                                context_map[parent_id]['children'].append(cat_id)
                            break
                else:
                    # Root level category - use itself as context
                    cat_info = self.category_resolver.get_category_info_by_id(taxonomy, cat_id)
                    if cat_info and cat_info.get('name'):
                        # FIX: Ensure keys are strings
                        str_cat_id = str(cat_id)
                        context_map[str_cat_id] = {
                            'info': {
                                'id': str_cat_id,
                                'name': cat_info['name'],
                        ...
                        if current_category and hasattr(current_category, 'parent_path') and current_category.parent_path:
                            ...
                            for node in self._traverse_taxonomy(taxonomy):
                                if (hasattr(node, 'name') and node.name == immediate_parent_name and
                                    ...
                                    if parent_cat_info and parent_cat_info.get('name'):
                                        parent_id = str(node.id)
                                        if parent_id not in parent_child_structure:
                                        ...
                                        if child_info and child_info.get('name'):
                                            parent_child_structure[parent_id]['children'].append({
                                                'id': cat_id,
                                                ...
                                                if isinstance(batch_result, dict):
                                                    safe_log_start(
                                                        "gemini_batch_result_debug",
                                                        {
                                                            ...
                                                            "expected_categories": [c['id'] for c in category_info if c.get('is_selected')],
                                                            "chunk_id": chunk_id
                                                        },
                                                        project_id
                                                    )
...
JSON Response:"""
                    ...
                    for cat_info in category_info:
                        ...
                        # Extract results for this selected category from the batch response
                        category_result = batch_result.get(cat_id, {})
