...
from ..services.job_monitoring_service import wait_for_upload_completion, wait_for_upload_completion_with_paths
...
def parse_selected_documents(project_id: int, uploaded_files: List, parser: str, selected_document_ids: Optional[List[int]] = None, pages_per_split: int = 5):
    ...
    try:
        ...
        st.info("📊 You can monitor the job progress in the job status section below.")
        st.info("🔄 The document list will update automatically when parsing completes.")
...
def chunk_parsed_documents(project_id: int, parsed_docs: List[dict], chunker: str, chunk_strategy: str,
    ...
    try:
        ...
        st.info("📊 You can monitor the job progress in the job status section.")
        st.info("🔄 Refreshing document list...")
        st.rerun()  # Refresh to show updated status
