...
from ..services.job_monitoring_service import display_processing_status
...
def render_document_processing():
    ...
    with tab_chunk:
        ...
        # Display processing status if there's an active job
        if hasattr(session_state, 'processing_job_id') and session_state.processing_job_id:
            st.markdown("### 📊 Parsing Job Status")
            ...
            # Show job ID
            status_placeholder.markdown(f"**Job ID:** {session_state.processing_job_id}")

            try:
                status_response = api_client.get(f"/api/v1/jobs/status/{session_state.processing_job_id}")
                if status_response:
                    ...
                    if status == "running":
                        status_placeholder.markdown("🔄 **Job Running**")
                        progress_placeholder.progress(progress / 100)
                        if current_step:
                            step_placeholder.text(f"Current Step: {current_step}")
                        else:
                            step_placeholder.empty()

                        # Auto-refresh every 5 seconds while job is running
                        auto_refresh_placeholder.info("🔄 Auto-refreshing every 5 seconds...")
                        ...
                        status_placeholder.markdown("✅ **Job Completed**")
                        progress_placeholder.empty()
                        step_placeholder.empty()
                        auto_refresh_placeholder.success("Job completed! Documents are now available.")

                        # Clear the job ID since it's done
                        session_state.processing_job_id = None

                        # Set flag to trigger document list refresh
                        session_state.job_just_completed = True

                        # Small delay to show completion message, then refresh
                        time.sleep(2)
                        st.rerun()  # Refresh to show updated document list

                    elif status == "failed":
                        status_placeholder.markdown("❌ **Job Failed**")
                        progress_placeholder.empty()
                        step_placeholder.empty()
                        ...
                        status_placeholder.markdown("⏳ **Job Pending**")
                        progress_placeholder.empty()
                        step_placeholder.empty()
                        auto_refresh_placeholder.info("🔄 Auto-refreshing every 5 seconds...")
                        ...
                        status_placeholder.markdown(f"❓ **Job {status.title()}**")
                        progress_placeholder.empty()
                        step_placeholder.empty()
                        auto_refresh_placeholder.empty()

            except Exception as e:
                status_placeholder.warning(f"Could not check job status: {e}")
                progress_placeholder.empty()
                step_placeholder.empty()
                auto_refresh_placeholder.empty()
        ...
        elif hasattr(session_state, 'processing_job_id') and session_state.processing_job_id:
            docs_placeholder.info("🔄 Waiting for parsing job to complete...")
            # Auto-refresh document list every 3 seconds while job is running
            time.sleep(3)
            st.rerun()
        else:
