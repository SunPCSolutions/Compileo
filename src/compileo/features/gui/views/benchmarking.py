        ...
        try:
            ...
            try:
                ...
                if submitted_id:
                    ...
                    show_benchmark_progress(submitted_id)
                else:
...
def show_benchmark_progress(job_id: str):
    ...
    # Check status once before loop to show stop button
    try:
        db_conn = get_db_connection()
        cursor = db_conn.cursor()
        cursor.execute("SELECT status FROM benchmark_jobs WHERE id = ?", (job_id,))
        row = cursor.fetchone()
        
        if row and row[0] in ["pending", "running"]:
            if st.button("🛑 Stop Job", key=f"stop_{job_id}"):
                try:
                    # Import here to avoid circular imports
                    from ...features.jobhandle.enhanced_job_queue import enhanced_job_queue_manager
                    if enhanced_job_queue_manager.cancel_job(job_id):
            ...
            try:
                ...
                if row:
                    ...
                    if status_value == "completed":
                        progress_placeholder.success("✅ Benchmarking completed!")
                        ...
                        if result_row:
                            ...
                            st.warning("Job completed but no results were generated. This usually indicates an issue with the benchmark execution (e.g., dataset loading failure).")
                        break
                    elif status_value == "failed":
                        progress_placeholder.error("❌ Benchmarking failed!")
                        if error_msg:
                            st.error(f"Error: {error_msg}")
                        break
                    elif status_value == "cancelled":
                        progress_placeholder.warning("⚠️ Benchmarking cancelled.")
                        break
                    else:
                        # Show progress - for now, just show status
                        progress_placeholder.progress(0.5)  # Placeholder progress
                        status_placeholder.info(f"Status: {status_value.title()}")
                        eta_placeholder.caption("Processing...")
                else:
                    progress_placeholder.error("Job not found")
                    break

            except Exception as e:
                progress_placeholder.error(f"Failed to get progress: {str(e)}")
                break
            ...
            try:
                ...
                if row:
                    ...
                    if status_value == "completed":
                        ...
                        # Show stop button for manual refresh mode too
                        if st.button("🛑 Stop Job", key=f"stop_manual_{job_id}"):
                            try:
                                from ...features.jobhandle.enhanced_job_queue import enhanced_job_queue_manager
                                if enhanced_job_queue_manager.cancel_job(job_id):
                                    st.warning("Job cancellation requested...")
