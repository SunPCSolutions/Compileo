"""
AI-powered taxonomy generation using Google Gemini.
"""
...
class GeminiTaxonomyGenerator:
    """
    Generates hierarchical taxonomies from document chunks using Google Gemini.
    """
    ...
    def _generate_single_pass(
        ...
        taxonomy_data = self._call_gemini_for_taxonomy(chunks, domain, depth, category_limits, specificity_level)
    ...
    def _call_gemini_for_taxonomy(self, chunks: List[str], domain: str, depth: Optional[int], category_limits: Optional[List[int]] = None, specificity_level: int = 1) -> str:
        """
        Call Gemini API to generate taxonomy from chunks.
...
- Make category names specific and actionable"""
    ...
    def _parse_taxonomy_response(self, response_text: str) -> Dict[str, Any]:
        """
        Parse and validate Gemini's taxonomy response.
        ...
        """
        try:
            # Parse JSON directly from Gemini response
    ...
    def extend_taxonomy(
        ...
        Args:
            ...
            domain: Knowledge domain (medical, legal, technical, etc.)
            sample_size: Number of chunks to sample (None = use all)
        ...
        if batch_size and len(chunks) > batch_size:
            ...
            chunks = ChunkSampler.sample_chunks(chunks, batch_size)

        # Generate extended taxonomy using Gemini
        taxonomy_data = self._call_gemini_for_taxonomy_extension(
    ...
    def _call_gemini_for_taxonomy_extension(
        self,
        existing_taxonomy: Dict[str, Any],
        chunks: List[str],
        ...
        """
        Call Gemini API to extend an existing taxonomy.
...
- Preserve the existing taxonomy structure and add subcategories to appropriate nodes"""
    ...
    def _parse_taxonomy_extension_response(self, response_text: str, existing_taxonomy: Dict[str, Any]) -> Dict[str, Any]:
        """
        Parse and validate Gemini's taxonomy extension response.
